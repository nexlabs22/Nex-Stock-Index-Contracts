...
contract IndexFactoryBalancer is Initializable, OwnableUpgradeable, PausableUpgradeable {
    struct ActionInfo {
        uint256 actionType;
        uint256 nonce;
    }
        ...
        for (uint256 i; i < factoryStorage.totalCurrentList(); i++) {
            ...
            if (tokenValuePercent > factoryStorage.tokenOracleMarketShare(tokenAddress)) {
                uint256 amount = tokenBalance
                    - (tokenBalance * factoryStorage.tokenOracleMarketShare(tokenAddress) / tokenValuePercent);
                ...
                uint256 shortagePercent = factoryStorage.tokenOracleMarketShare(tokenAddress) - tokenValuePercent;
    ...
    function firstRebalanceAction() public onlyOwner returns (uint256) {
        rebalanceNonce += 1;
    ...
    function secondRebalanceAction(uint256 _rebalanceNonce) public onlyOwner {
    ...
    function completeRebalanceActions(uint256 _rebalanceNonce) public onlyOwner {
